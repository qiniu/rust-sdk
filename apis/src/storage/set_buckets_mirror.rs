...
pub struct PathParams {
    ...
    r#src_site_url: Option<std::borrow::Cow<'static, str>>,
    r#host: Option<std::borrow::Cow<'static, str>>,
...
impl PathParams {
    ...
    fn build(self) -> Vec<std::borrow::Cow<'static, str>> {
        ...
        if let Some(segment) = self.r#src_site_url {
            all_segments.push(std::borrow::Cow::Borrowed("from"));
            all_segments.push(segment);
        }
        if let Some(segment) = self.r#host {
            all_segments.push(std::borrow::Cow::Borrowed("host"));
            all_segments.push(segment);
        }
...
impl PathParams {
    ...
    pub fn set_src_site_url_as_str(
        ...
        self.r#src_site_url = Some(qiniu_utils::base64::urlsafe(value.into().as_bytes()).into());
        self
    }
    #[inline]
    #[doc = "回源时使用的 `Host` 头部值"]
...
impl<'client> Client<'client> {
    ...
    pub fn new_request(
        ...
        SyncRequestBuilder(
            self.0
                .post(&[qiniu_http_client::ServiceName::Uc], into_endpoints.into())
    ...
    pub fn new_async_request(
        ...
        AsyncRequestBuilder(
            self.0
                .async_post(&[qiniu_http_client::ServiceName::Uc], into_endpoints.into())
                .authorization(qiniu_http_client::Authorization::v2(credential))
                .idempotent(qiniu_http_client::Idempotent::Always)
                .path(crate::base_utils::join_path(
                    "/image",
