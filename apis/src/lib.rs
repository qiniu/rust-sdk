...
#![doc = r" 例如可以通过启用 `ureq` 功能导入 `qiniu-http-ureq` 库作为 HTTP 客户端，"]
#![doc = r" 通过启用 `reqwest` 功能导入 `qiniu-http-reqwest` 库作为 HTTP 客户端，"]
#![doc = r" 通过启用 `isahc` 功能导入 `qiniu-http-isahc` 库作为 HTTP 客户端。"]
