...
impl File {
    ...
    #[inline]
    pub async fn create(path: impl AsRef<StdPath>) -> IoResult<File> {
        fs::File::create(path.as_ref()).await.map(File)
    }
}
...
pub async fn canonicalize(path: impl AsRef<StdPath>) -> IoResult<StdPathBuf> {
