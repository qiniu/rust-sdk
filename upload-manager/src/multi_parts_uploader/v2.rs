        ...
        fn _could_recover<R: ResumableRecorder>(
            ...
            initialized.recovered_records.take(offset).and_then(|record| {
                if record.size == part_size
                    ...
                    && Some(record.response_body.get_md_5_as_str()) == md5_of_sync_reader(data_reader).ok().as_deref()
                {
...
#[cfg(feature = "async")]
async fn md5_of_async_reader<R: AsyncRead + AsyncReset + Unpin>(reader: &mut R) -> IoResult<String> {
    use futures::{
        ...
        fn _try_to_recover<R: ResumableRecorder>(
            ...
            let (upload_id, mut records) = {
                ...
                let upload_id = if let Some(line) = lines.next() {
                    ...
                    if !header.is_v2() || header.initialized_at() + uploaded_part_ttl <= SystemTime::now() {
                        return Ok(None);
                    }
                    header.upload_id
                } else {
